...
export default function PazaakGame() {
  ...
  // Load player deck from sessionStorage or use default
  const getPlayerDeck = (): SideCard[] => {
    const savedDeck = sessionStorage.getItem('playerDeck');
    if (savedDeck) {
      const fullDeck = JSON.parse(savedDeck);
      // Randomly select 4 cards from the 8-card deck
  ...
  const startNewMatch = () => {
    startGame();
    setPlayerSideHand([...getPlayerDeck()]);
  ...
  // Auto-start match when component mounts
  useEffect(() => {
    startNewMatch();
  }, []);

  const handleReturnToMenu = () => {
    playSound(buttonClickRef);
